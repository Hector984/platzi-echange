<template>
  <div class="flex-col">
    <div class="flex justify-center">
      <bounce-loader v-bind:loading="isLoading" v-bind:color="'#68d391'" v-bind:size="100"></bounce-loader>
        <px-assets-table v-if="!isLoading" ></px-assets-table>
    </div>
    <template v-if="!isLoading">
      <div class="flex flex-col sm:flex-row justify-around items-center">
        <div class="flex flex-col items-center">
          <img v-bind:src="`https://static.coincap.io/assets/icons/${asset.symbol.toLowerCase()}@2x.png`" class="w-20 h-20 mr-5" />
          <h1 class="text-5xl">
            {{ asset.name }}
      ...
      v-bind:min="min" 
      v-bind:max="max"
      v-bind:data="history.map(h => [h.date, parseFloat(h.priceUsd).toFixed(2)])"></line-chart>
      ...
      <table>
        <tr v-for="m in markets" v-bind:key="`${m.exchangeId}-${m.priceUsd}`" class="border-b">
          ...
          <td>
            <px-button v-bind:is-loading="m.isLoading || false" v-if="!m.url" v-on:custom-click="getWebSite(m)">
